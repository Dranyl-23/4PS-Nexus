    ...
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900">Compliance & Requirements</h1>
        </div>
        <p className="text-slate-500">Track your family's 4Ps program conditional requirements.</p>
      </div>

      {/* Status Banner */}
      <div className={`p-6 rounded-2xl border ${allCompliant ? 'bg-emerald-50 border-emerald-100' : 'bg-amber-50 border-amber-100'} flex flex-col md:flex-row items-start md:items-center gap-4`}>
        <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${allCompliant ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
          {allCompliant ? <CheckCircle2 className="w-6 h-6" /> : <AlertCircle className="w-6 h-6" />}
        </div>
        <div className="flex-1">
          <h3 className={`font-bold text-lg mb-1 ${allCompliant ? 'text-emerald-900' : 'text-amber-900'}`}>
      ...
      <div className="grid gap-4 md:gap-6">
        {REQUIREMENTS.map((req) => {
          const Icon = req.icon;
          return (
            <div key={req.id} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex flex-col md:flex-row gap-6">
                
                {/* Left side: Icon & Text */}
                <div className="flex-1 flex gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${req.bgColor} ${req.color}`}>
                  ...
                  <div>
                    ...
                    {req.date && (
                      <div className="flex items-center gap-1.5 text-sm font-medium text-amber-600 mt-3 bg-amber-50 px-3 py-1.5 rounded-lg w-fit">
